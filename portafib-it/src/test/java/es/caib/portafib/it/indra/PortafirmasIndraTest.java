...
import es.caib.portafib.it.indra.enterprise.DownloadDocumentPortafirmasHandler;
import es.caib.portafib.it.indra.enterprise.EnvioPortafirmasHandler;
import es.caib.portafib.it.indra.helium.DocumentPortasignatures;
import es.caib.portafib.it.indra.helium.PasSignatura;
import es.caib.portafib.it.indra.helium.PortasignaturesPluginCaib;
...
public class PortafirmasIndraTest extends IndraTestUtils {
  ...
  protected static void test(final CodeBase codeBase) throws Exception {
    ...
    try {
      ...
      if (codeBase.equals(CodeBase.HELIUM)) {
        callDownloadFile_Helium(proxy, app, peticioID.intValue(), null, uploadedDocuments);
      } else {
        callDownloadFile_Enterprise(proxy, app, peticioID, null, uploadedDocuments);
      }
  ...
  private static DocInfo createDocToSign() throws IOException {
    ...
    InputStream inputStream =  PortafirmasIndraTest.class.getResourceAsStream("/testfiles/" + fileName);
  ...
  private static DocInfo createAnnex() throws IOException {
    ...
    InputStream inputStream =  PortafirmasIndraTest.class.getResourceAsStream("/testfiles/" + annexName);
    byte[] arxiuContingut = readFully(inputStream);
