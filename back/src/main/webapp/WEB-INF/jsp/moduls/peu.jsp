<%@page import="es.caib.portafib.utils.Build"%><%@page
    import="es.caib.portafib.logic.utils.LogicUtils"%><%@ page
    ...
    <div class="row mr-auto ml-3 mr-3 peuResponsive">
        ...
        <div class="col-4 pt-2 elementPeuResponsive">
            <b title="Build: <%=LogicUtils.getBuild()%>"><fmt:message
                    key="app.nom" /> v<%=LogicUtils.getVersio()%></b><br />
            <i><a
        ...
        <div
            ...
            <c:if test="${not empty loginInfo.entitatID}">
                <c:out value="${loginInfo.entitat.adrezaHtml}"
                    escapeXml="false" />
            </c:if>
        </div>
        ...
        <c:if test="${not empty loginInfo.entitatID}">
            ...
            <div id="modalAjuda" class="modal hide fade" tabindex="-1"
                role="dialog"
                aria-labelledby="<fmt:message key="ajuda.titol" />"
                aria-hidden="true">
                ...
                <div class="modal-body">
                    ...
                    <ul>
                        <c:if
                            test="${not empty loginInfo.entitat.suportTelefon }">
                            <li><fmt:message key="ajuda.viatelefon" />
                                ${loginInfo.entitat.suportTelefon}</li>
                        </c:if>

                        <c:if
                            test="${not empty loginInfo.entitat.suportWeb }">
                        ...
                        <c:if
                            test="${not empty loginInfo.entitat.suportEmail }">
