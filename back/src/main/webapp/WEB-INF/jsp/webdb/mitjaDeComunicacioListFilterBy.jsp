...
<un:useConstants var="MitjaDeComunicacioFields" className="es.caib.portafib.model.fields.MitjaDeComunicacioFields"/>
  ...
  <div id="FilterDiv" class="well formbox" style="${displayFilterDiv} margin-bottom:3px; margin-left: 1px; padding:3px;">
      ...
      <div class="form-inline">
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,MitjaDeComunicacioFields.MITJADECOMUNICACIOID)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="mitjaDeComunicacio.mitjaDeComunicacioID" var="mitjaDeComunicacioID" />
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,MitjaDeComunicacioFields.ACTIU)}">
            <%-- FILTRE NUMERO --%>      
            <div class="input-prepend input-append" style="padding-right: 4px;padding-bottom: 4px;">
              <span class="add-on"><fmt:message key="mitjaDeComunicacio.actiu" />:</span>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,MitjaDeComunicacioFields.JAVACLASS)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="mitjaDeComunicacio.javaClass" var="javaClass" />
              <fmt:message key="genapp.form.searchby" var="cercaperjavaClass" >                
                 <fmt:param value="${javaClass}"/>
              </fmt:message>
              <span class="add-on"><c:out value="${javaClass}" />:</span>
              <form:input cssClass="search-query input-medium" placeholder="${cercaperjavaClass}" path="javaClass" />
            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,MitjaDeComunicacioFields.DESCRIPCIO)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="mitjaDeComunicacio.descripcio" var="descripcio" />
