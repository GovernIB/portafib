...
<%@ include file="/WEB-INF/jsp/moduls/includes.jsp"%>
          ...
          <c:if test="${__theFilterForm.editButtonVisible || __theFilterForm.deleteButtonVisible || not empty __theFilterForm.additionalButtons || not empty __theFilterForm.additionalButtonsByPK}">
          <td>
            <div class="btn-group" data-toggle="buttons-checkbox">
            <c:if test="${__theFilterForm.editButtonVisible}">
            <a class="btn" href="#" 
              onclick="goTo('<c:url value="${contexte}/${peticioDeFirmaUsuariEntitat.peticioDeFirmaID}/edit"/>')"
            ...
            <a class="btn btn-danger" href="#myModal"
               onclick="openModal('<c:url value="${contexte}/${peticioDeFirmaUsuariEntitat.peticioDeFirmaID}/delete"/>','show');"
               title="<fmt:message key="genapp.delete"/>">
               <i class="icon-trash icon-white"></i>
            </a>
            </c:if>
            <c:set var="bracket" value="{0}"/>
            <c:set var="pk" value="${peticioDeFirmaUsuariEntitat.peticioDeFirmaID}"/>
