...
<div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
  ...
  <input id="${__entry.value.searchBy.fullName}" name="${__entry.value.searchBy.fullName}" class="search-query input-medium" placeholder="${cercaperAF}" type="text" value="${__entry.value.searchByValue}"/>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,RoleUsuariAplicacioFields.ID)}">
            <%-- FILTRE NUMERO --%>      
            <div class="input-prepend input-append" style="padding-right: 4px;padding-bottom: 4px;">
              <span class="add-on"><fmt:message key="roleUsuariAplicacio.id" />:</span>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,RoleUsuariAplicacioFields.ROLEID)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="roleUsuariAplicacio.roleID" var="roleID" />
              <fmt:message key="genapp.form.searchby" var="cercaperroleID" >                
                 <fmt:param value="${roleID}"/>
              </fmt:message>
              <span class="add-on"><c:out value="${roleID}" />:</span>
              <form:input cssClass="search-query input-medium" placeholder="${cercaperroleID}" path="roleID" />
            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,RoleUsuariAplicacioFields.USUARIAPLICACIOID)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="roleUsuariAplicacio.usuariAplicacioID" var="usuariAplicacioID" />
