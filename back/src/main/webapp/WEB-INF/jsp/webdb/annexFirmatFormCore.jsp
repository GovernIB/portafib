...
<un:useConstants var="AnnexFirmatFields" className="es.caib.portafib.model.fields.AnnexFirmatFields"/>
        ...
        <tr id="annexFirmat_annexID_rowid">
          ...
          <input type="text" readonly="true" class="form-control input-xxlarge uneditable-input" value="${gen:findValue(__theForm.annexFirmat.annexID,__theForm.listOfAnnexForAnnexID)}"  />
          </c:if>
          <c:if test="${!gen:contains(__theForm.readOnlyFields ,AnnexFirmatFields.ANNEXID)}" >
          <c:set var="containEmptyValue"  value="false" />
          <form:select id="annexFirmat_annexID"  onchange="if(typeof onChangeAnnexID == 'function') {  onChangeAnnexID(this); };"  cssClass="form-control col-md-8" path="annexFirmat.annexID">
        ...
        <tr id="annexFirmat_firmaID_rowid">
          ...
          <input type="text" readonly="true" class="form-control input-xxlarge uneditable-input" value="${gen:findValue(__theForm.annexFirmat.firmaID,__theForm.listOfFirmaForFirmaID)}"  />
          </c:if>
          <c:if test="${!gen:contains(__theForm.readOnlyFields ,AnnexFirmatFields.FIRMAID)}" >
          <c:set var="containEmptyValue"  value="false" />
          <form:select id="annexFirmat_firmaID"  onchange="if(typeof onChangeFirmaID == 'function') {  onChangeFirmaID(this); };"  cssClass="form-control col-md-8" path="annexFirmat.firmaID">
