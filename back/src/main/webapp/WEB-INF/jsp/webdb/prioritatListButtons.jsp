...
<%@ include file="/WEB-INF/jsp/moduls/includes.jsp"%>
          ...
          <td>
          <c:set var="pk" value="${prioritat.prioritatID}"/>
          <c:choose>
           <c:when test="${__theFilterForm.actionsRenderer == 1}">
            <div class="btn-group" data-toggle="buttons-checkbox">
            <c:if test="${__theFilterForm.editButtonVisible}">
            <a class="btn " href="<c:url value="${contexte}/${prioritat.prioritatID}/edit"/>" onclick="goTo('<c:url value="${contexte}/${prioritat.prioritatID}/edit"/>')" title="<fmt:message key="genapp.edit"/>">
               <i class="icon-pencil"></i>
            </a>
            </c:if>
            <c:if test="${__theFilterForm.deleteButtonVisible}">
            <a class="btn btn-danger" href="#myModal" onclick="openModal('<c:url value="${contexte}/${prioritat.prioritatID}/delete"/>','show');" title="<fmt:message key="genapp.delete"/>">
      ...
      <ul class="dropdown-menu pull-right" style="min-width:35px;padding:5px 5px 0px 5px;margin:0px;font-size: 12px" >
            <c:if test="${__theFilterForm.editButtonVisible}">
            <li>
            <a class="btn  btn-small a_item" style="margin-bottom:5px;${(empty )? '' : 'color: white;'}" href="<c:url value="${contexte}/${prioritat.prioritatID}/edit"/>" onclick="goTo('<c:url value="${contexte}/${prioritat.prioritatID}/edit"/>')">
            <i class="icon-pencil"></i>
            ...
            <c:if test="${__theFilterForm.deleteButtonVisible}">
            <li>
            <a class="btn btn-danger btn-small a_item" style="margin-bottom:5px;${(empty btn-danger)? '' : 'color: white;'}" href="#myModal" onclick="openModal('<c:url value="${contexte}/${prioritat.prioritatID}/delete"/>','show');">
            <i class="icon-trash icon-white"></i>
