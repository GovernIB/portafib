...
<un:useConstants var="UsuariEntitatFavoritFields" className="es.caib.portafib.model.fields.UsuariEntitatFavoritFields"/>
        ...
        <tr id="usuariEntitatFavorit_origenID_rowid">
          ...
          <input type="text" readonly="true" class="form-control input-xxlarge uneditable-input" value="${gen:findValue(__theForm.usuariEntitatFavorit.origenID,__theForm.listOfUsuariEntitatForOrigenID)}"  />
          </c:if>
          <c:if test="${!gen:contains(__theForm.readOnlyFields ,UsuariEntitatFavoritFields.ORIGENID)}" >
          <c:set var="containEmptyValue"  value="false" />
          <form:select id="usuariEntitatFavorit_origenID"  onchange="if(typeof onChangeOrigenID == 'function') {  onChangeOrigenID(this); };"  cssClass="form-control col-md-8" path="usuariEntitatFavorit.origenID">
        ...
        <tr id="usuariEntitatFavorit_favoritID_rowid">
          ...
          <input type="text" readonly="true" class="form-control input-xxlarge uneditable-input" value="${gen:findValue(__theForm.usuariEntitatFavorit.favoritID,__theForm.listOfUsuariEntitatForFavoritID)}"  />
          </c:if>
          <c:if test="${!gen:contains(__theForm.readOnlyFields ,UsuariEntitatFavoritFields.FAVORITID)}" >
          <c:set var="containEmptyValue"  value="false" />
          <form:select id="usuariEntitatFavorit_favoritID"  onchange="if(typeof onChangeFavoritID == 'function') {  onChangeFavoritID(this); };"  cssClass="form-control col-md-8" path="usuariEntitatFavorit.favoritID">
