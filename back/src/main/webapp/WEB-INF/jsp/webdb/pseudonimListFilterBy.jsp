...
<un:useConstants var="PseudonimFields" className="es.caib.portafib.model.fields.PseudonimFields"/>
  ...
  <div id="FilterDiv" class="wellgroupfilter formbox" style="${displayFilterDiv} margin-bottom:3px; margin-left: 1px; padding:3px;">
      ...
      <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,PseudonimFields.PSEUDONIMID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="pseudonim.pseudonimid" />:</span>

              <span class="add-on">&nbsp;<fmt:message key="genapp.from" /></span>
              
              <form:input cssClass="input-append input-small" path="pseudonimidDesde" />


              <span class="add-on">&nbsp;<fmt:message key="genapp.to" />&nbsp;</span>

              <form:input cssClass="input-append input-small search-query" path="pseudonimidFins" />

            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,PseudonimFields.PSEUDONIM)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="pseudonim.pseudonim" var="pseudonim" />
              <fmt:message key="genapp.form.searchby" var="cercaperpseudonim" >                
                 <fmt:param value="${pseudonim}"/>
              </fmt:message>
              <span class="add-on"><c:out value="${pseudonim}" />:</span>
              <form:input cssClass="search-query input-medium" placeholder="${cercaperpseudonim}" path="pseudonim" />
            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,PseudonimFields.NIF)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="pseudonim.nif" var="nif" />
