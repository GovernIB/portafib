...
<un:useConstants var="ModulDeFirmaFields" className="es.caib.portafib.model.fields.ModulDeFirmaFields"/>
  ...
  <c:set var="__theForm" value="${modulDeFirmaForm}"/>
  ...
  <div class="module_content">
    ...
    <c:forEach items="${__theForm.hiddenFields}" var="hiddenFieldF" >
      ...
      <c:if test="${gen:hasProperty(__theForm.modulDeFirma,hiddenField)}">
        <form:errors path="modulDeFirma.${hiddenField}" cssClass="errorField alert alert-error" />
        <form:hidden path="modulDeFirma.${hiddenField}"/>
      </c:if>
    </c:forEach>
