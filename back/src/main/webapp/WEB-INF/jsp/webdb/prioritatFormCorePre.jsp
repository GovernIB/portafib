...
<un:useConstants var="PrioritatFields" className="es.caib.portafib.model.fields.PrioritatFields"/>
  ...
  <c:set var="__theForm" value="${prioritatForm}"/>
  ...
  <div class="module_content">
    ...
    <c:forEach items="${__theForm.hiddenFields}" var="hiddenFieldF" >
      ...
      <c:if test="${gen:hasProperty(__theForm.prioritat,hiddenField)}">
        <form:errors path="prioritat.${hiddenField}" cssClass="errorField alert alert-error" />
        <form:hidden path="prioritat.${hiddenField}"/>
      </c:if>
    </c:forEach>
