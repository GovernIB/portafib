...
public class GestioEntitatAdminController extends EntitatController implements ConstantsV2, ConstantsPortaFIB {
    ...
    public EntitatForm getEntitatForm(EntitatJPA _jpa, boolean __isView, HttpServletRequest request, ModelAndView mav)
        ...
        if (entitatForm.isNou()) {
            ...
            entitatForm.getEntitat().setPoliticaTaulaFirmes(
                    ConstantsPortaFIB.POLITICA_TAULA_FIRMES_OPCIONAL_PER_DEFECTE_DEFINIT_EN_CONF);
            ...
            // TODO S'ha d'ocultar només si te usuarisEntitat o usuarisAplicacio associats
            // (veure mètode delete)
    ...
    public EntitatFilterForm getEntitatFilterForm(Integer pagina, ModelAndView mav, HttpServletRequest request)
         ...
         * representa la composición básica (títulos, botones columnas, etc...) de un
         * listado. Durante la sesión el usuario puede des/ordenar o des/agrupar como el
         * quiera pero, a pesar de estas acciones la estructura base se debe mantener.
         * Por ello, mediante la siguiente condición inicializamos la composición del
         * listado base solo una vez: isNou()
         */
    ...
    @Override
    public List<StringKeyValue> getReferenceListForUsuariAplicacioID(HttpServletRequest request, ModelAndView mav,
            EntitatFilterForm entitatFilterForm, List<Entitat> list, Map<Field<?>, GroupByItem> _groupByItemsMap,
            Where w) throws I18NException {
    ...
    public List<StringKeyValue> getReferenceListForCustodiaInfoID(HttpServletRequest request, ModelAndView mav,
        ...
        if (entitatForm.isNou()) {
            ...
            where2 = Where.AND(where, CustodiaInfoFields.ENTITATID.equal(entitatForm.getEntitat().getEntitatID()),
     ...
     */
    @Override
    public List<StringKeyValue> getReferenceListForPluginSegellTempsID(HttpServletRequest request, ModelAndView mav,
            EntitatForm entitatForm, Where where) throws I18NException {

        Where where2 = Where.AND(where, PluginFields.TIPUS.equal(ConstantsV2.TIPUS_PLUGIN_SEGELLDETEMPS),
    ...
    public List<StringKeyValue> getReferenceListForPoliticaCustodia(HttpServletRequest request, ModelAndView mav,
            Where where) throws I18NException {
        List<StringKeyValue> kvList = staticGetReferenceListForPoliticaCustodia(
                POLITICA_CUSTODIA.POLITICA_CUSTODIA_ENTITAT);
        return kvList;
    }
     ...
     */
    @Override
    public List<StringKeyValue> getReferenceListForPosicioTaulaFirmes(HttpServletRequest request, ModelAndView mav,
            Where where) throws I18NException {
     ...
     * 
     * 
     * INSERT INTO pfi_propietatglobal(entitatid, clau, valor, descripcio) SELECT
     * entitatid, '', '2', '' FROM pfi_entitat;
     * 
     * INSERT INTO pfi_propietatglobal(entitatid, clau, valor, descripcio) SELECT
     * entitatid, '', NULL, '' FROM pfi_entitat;
     * 
     */
    ...
    public EntitatJPA create(HttpServletRequest request, EntitatJPA entitat)
        ...
        for (Map.Entry<String, Propietat> entry : PropietatsConstants.propietatsEntitat.entrySet()) {
            ...
            try {
                ...
                propietatGlobalEjb.create(entry.getValue().clau, null, _entitatID_, entry.getValue().descripcio);

            } catch (I18NException ie) {
                String msg = I18NUtils.getMessage(ie);
                // TODO Traduir
                String missatge = "S'ha produït un error creant les propietats per l'entitat" + e.getNom()
                        + ". Haurà de crear-les manualment. Error: " + msg;
    ...
    // # 165
    @Override
    public void postValidate(HttpServletRequest request, EntitatForm entitatForm, BindingResult result)
            throws I18NException {
