...
public class SomDelegatDeDeleController extends DelegacioDestController
    ...
    public Where getAdditionalCondition(HttpServletRequest request) throws I18NException {
        Where w = Where.AND(COLABORADORDELEGATID.equal(LoginInfo.getInstance().getUsuariEntitatID()),
                ESDELEGAT.equal(esDelegat()));

        if (esDelegat()) {
            // No volem delegacions on fitxer encara no està firmat
            return Where.AND(w, FITXERAUTORITZACIOID.isNotNull());
        } else {
            return w;
        }
    ...
    public ColaboracioDelegacioFilterForm getColaboracioDelegacioFilterForm(Integer pagina, ModelAndView mav,
        ...
        if (colaboracioDelegacioFilterForm.isNou()) {
            ...
            colaboracioDelegacioFilterForm.addLabel(DESTINATARIID,
                    esDelegat() ? "delegatde.menu" : "colaboradorde.menu");
            colaboracioDelegacioFilterForm.getHiddenFields().remove(DESTINATARIID);

            colaboracioDelegacioFilterForm.addAdditionalButtonForEachItem(new AdditionalButton("fas fa-eye",
                    "genapp.viewtitle", getContextWeb() + "/view/{0}", AdditionalButtonStyle.INFO));
    ...
    public ColaboracioDelegacioForm getColaboracioDelegacioForm(ColaboracioDelegacioJPA _jpa, boolean __isView,
        ...
        List<Long> currentTipusDocument = getCurrentTipusDocument(colaboracioDelegacioForm.getColaboracioDelegacio(),
        ...
        colaboracioDelegacioForm.addAdditionalButton(
                new AdditionalButton("", "tornar", getContextWeb() + "/list", AdditionalButtonStyle.PRIMARY));
