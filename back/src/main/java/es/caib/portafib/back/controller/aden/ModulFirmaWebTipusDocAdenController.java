...
public class ModulFirmaWebTipusDocAdenController extends ModulDeFirmaPerTipusDeDocumentController {
    ...
    @Override
    public ModulDeFirmaPerTipusDeDocumentFilterForm getModulDeFirmaPerTipusDeDocumentFilterForm(Integer pagina,
            ModelAndView mav, HttpServletRequest request) throws I18NException {
        ModulDeFirmaPerTipusDeDocumentFilterForm modulDeFirmaPerTipusDeDocumentFilterForm;
        modulDeFirmaPerTipusDeDocumentFilterForm = super.getModulDeFirmaPerTipusDeDocumentFilterForm(pagina, mav,
                request);
        if (modulDeFirmaPerTipusDeDocumentFilterForm.isNou()) {
            ...
            modulDeFirmaPerTipusDeDocumentFilterForm.addAdditionalButton(new AdditionalButton("fas fa-plus-circle",
                    "modulfirmatipusdoc.novaentrada", getContextWeb() + "/new", AdditionalButtonStyle.SECONDARY));
    ...
    public ModulDeFirmaPerTipusDeDocumentForm getModulDeFirmaPerTipusDeDocumentForm(
            ModulDeFirmaPerTipusDeDocumentJPA _jpa, boolean __isView, HttpServletRequest request, ModelAndView mav)
            throws I18NException {
        ModulDeFirmaPerTipusDeDocumentForm modulDeFirmaPerTipusDeDocumentForm;
        modulDeFirmaPerTipusDeDocumentForm = super.getModulDeFirmaPerTipusDeDocumentForm(_jpa, __isView, request, mav);
    ...
    public List<StringKeyValue> getReferenceListForPluginID(HttpServletRequest request, ModelAndView mav, Where where)
        ...
        Where w = Where.AND(where, PluginFields.ENTITATID.equal(LoginInfo.getInstance().getEntitatID()),
                PluginFields.TIPUS.equal(ConstantsV2.TIPUS_PLUGIN_MODULDEFIRMA_WEB), PluginFields.ACTIU.equal(true));
    ...
    public List<StringKeyValue> getReferenceListForTipusDocumentID(HttpServletRequest request, ModelAndView mav,
        ...
        Where w = Where.AND(where, Where.OR(TipusDocumentFields.USUARIAPLICACIOID.equal(usrAppID),
