...
public class FirmaUsuariExternAdenController extends FirmaController {
    ...
    public Where getAdditionalCondition(HttpServletRequest request) throws I18NException {
        ...
        // Petició en marxa
        Where w4 = new FirmaQueryPath().BLOCDEFIRMES().FLUXDEFIRMES().PETICIODEFIRMA().TIPUSESTATPETICIODEFIRMAID()
                .equal(ConstantsV2.TIPUSESTATPETICIODEFIRMA_ENPROCES);
        // D'aquesta entitat
        Where w5 = new FirmaQueryPath().USUARIENTITAT().ENTITATID().equal(LoginInfo.getInstance().getEntitatID());
    ...
    public void init() {
        ...
        usuariEntitatRefList.setSelects(new Select<?>[] { new UsuariEntitatQueryPath().USUARIPERSONA().NIF().select });
        usuariEntitatRefList.setSeparator("");
    }

    @Override
    public FirmaFilterForm getFirmaFilterForm(Integer pagina, ModelAndView mav, HttpServletRequest request)
        ...
        if (firmaFilterForm.isNou()) {
            ...
            Set<Field<?>> hiddenFields = new HashSet<Field<?>>(Arrays.asList(FirmaFields.ALL_FIRMA_FIELDS));
            ...
            additionalField.setCodeName(PeticioDeFirmaFields._TABLE_MODEL + "." + PeticioDeFirmaFields._TABLE_MODEL);
            ...
            firmaFilterForm.addAdditionalButtonForEachItem(new AdditionalButton("fas fa-info", "veuredetalls",
                    getContextWeb() + "/veurepeticio/{0}", AdditionalButtonStyle.INFO));
            ...
            firmaFilterForm.addAdditionalButtonForEachItem(
                    new AdditionalButton("far fa-envelope", "firmausuariextern.reenviaremail",
                            getContextWeb() + "/reenviaremail/{0}", AdditionalButtonStyle.WARNING));
    ...
    @Override
    public FirmaForm getFirmaForm(FirmaJPA _jpa, boolean __isView, HttpServletRequest request, ModelAndView mav)
            throws I18NException {
    ...
    public String veurePeticio(@PathVariable("firmaID") Long firmaID, HttpServletRequest request,
        ...
        Field<Long> field = new FirmaQueryPath().BLOCDEFIRMES().FLUXDEFIRMES().PETICIODEFIRMA().PETICIODEFIRMAID();
    ...
    public String reenviarEmail(@PathVariable("firmaID") Long firmaID, HttpServletRequest request,
        ...
        try {
            ...
            peticioDeFirmaLogicaEjb.sendMailToExternalUser(LoginInfo.getInstance().getEntitatID(), peticioDeFirmaID,
                    titolPeticio, firmaJPA);

            HtmlUtils.saveMessageSuccess(request,
                    I18NUtils.tradueix("firmausuariextern.reenviar.ok", firmaJPA.getUsuariExternEmail()));
            ...
            HtmlUtils.saveMessageError(request, I18NUtils.tradueix("firmausuariextern.reenviar.error", email, error));
        }
    ...
    @Override
    public void postList(HttpServletRequest request, ModelAndView mav, FirmaFilterForm filterForm, List<Firma> list)
            throws I18NException {

        Map<Long, String> mapFirmaIDNomPeticio = (Map<Long, String>) filterForm.getAdditionalField(COLUMN_PETICIO_FIRMA)
                .getValueMap();
