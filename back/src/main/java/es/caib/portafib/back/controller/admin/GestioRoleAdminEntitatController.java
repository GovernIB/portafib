...
@MenuOption(group = Tab.MENU_ADMIN, labelCode = "administradorentitat.gestio", addSeparatorBefore = true, order = 20)
public class GestioRoleAdminEntitatController extends AbstractGestioRoleUsuariEntitatController {
    ...
    public void init() {
        ...
        this.personaRefList.setSelects(new Select<?>[] { UsuariPersonaFields.LLINATGES.select, new SelectConstant(", "),
    ...
    protected SeleccioUsuariForm getSeleccioUsuariForm(HttpServletRequest request) {
        ...
        if (LoginInfo.getInstance().getUsuariEntitatID() != null) {
            ...
            try {
                skvList = SearchJSONController.favoritsToUsuariPersona(usuariEntitatLogicaEjb
    ...
    protected String checksPostNif(HttpServletRequest request, UsuariPersona usuariPersona, String param1,
        ...
        if (entitatID != null && entitatID.trim().length() != 0) {
            ...
            if (entitat == null) {
                ...
                throw new I18NException("error.notfound", new I18NArgumentCode("entitat.entitat"),
                        new I18NArgumentCode("entitat.entitatID"), new I18NArgumentString(String.valueOf(entitatID)));
            } else {
                // L'entitat existeix i ara mira si existeix l'usuari-entitat
                UsuariEntitatJPA usuariEntitat;
                usuariEntitat = usuariEntitatLogicaEjb.findUsuariEntitatByUsername(entitatID, usuariPersonaID);
                ...
                if (usuariEntitat == null) {
                    ...
                    if (roleUsuariEntitatLogicaEjb.count(whereCount) != 0) {
                        ...
                        throw new I18NException("administradorentitat.jaexisteix", Utils.getNom(usuariPersona),
                                entitatID);
                    }
                }
            ...
            subquery = roleUsuariEntitatLogicaEjb.getSubQuery(RoleUsuariEntitatFields.USUARIENTITATID,
                    Where.AND(RoleUsuariEntitatFields.USUARIENTITATID.equal(UsuariEntitatFields.USUARIENTITATID),
                            RoleUsuariEntitatFields.ROLEID.equal(Constants.ROLE_ADEN)));

            Where wTots = Where.AND(UsuariEntitatFields.USUARIPERSONAID.equal(usuariPersonaID),
                    UsuariEntitatFields.CARREC.isNull(), UsuariEntitatFields.ACTIU.equal(true));

            Where wTotsSenseAden = Where.AND(wTots, UsuariEntitatFields.USUARIENTITATID.notIn(subquery));
    ...
    @Override
    public void preList(HttpServletRequest request, ModelAndView mav, RoleUsuariEntitatFilterForm filterForm)
            throws I18NException {
        ...
        if (orderBy != null && orderBy.equals(USUARIENTITATID.javaName)) {
            ...
            filterForm.setDefaultOrderBy(new OrderBy[] {
                    new OrderBy(new RoleUsuariEntitatQueryPath().USUARIENTITAT().USUARIPERSONA().LLINATGES(),
    ...
    @Override
    public void postList(HttpServletRequest request, ModelAndView mav, RoleUsuariEntitatFilterForm filterForm,
            List<RoleUsuariEntitat> list) throws I18NException {
    ...
    public Map<Field<?>, GroupByItem> fillReferencesForList(RoleUsuariEntitatFilterForm filterForm,
            HttpServletRequest request, ModelAndView mav, List<RoleUsuariEntitat> list, List<GroupByItem> groupItems)
            throws I18NException {
        ...
        _listSKV = this.personaRefList.getReferenceList(UsuariPersonaFields.USUARIPERSONAID, null,
