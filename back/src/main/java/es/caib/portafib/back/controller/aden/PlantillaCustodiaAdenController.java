...
@MenuOption(group = Tab.MENU_ADEN, labelCode = "plantillacustodia.gestio", order = 150)
public class PlantillaCustodiaAdenController extends CustodiaInfoController {
    ...
    public static void addHelp(CustodiaInfoForm custodiaInfoForm) {
        custodiaInfoForm.addHelpToField(MISSATGE, I18NUtils.tradueix("custodiainfo.missatge.ajuda"));
    ...
    public Where getAdditionalCondition(HttpServletRequest request) throws I18NException {
        return Where.AND(ENTITATID.equal(LoginInfo.getInstance().getEntitatID()), NOMPLANTILLA.isNotNull());
    }

    @Override
    public void postValidate(HttpServletRequest request, CustodiaInfoForm custodiaInfoForm, BindingResult result)
        ...
        if (nom == null || nom.trim().length() == 0) {
            result.rejectValue(get(NOMPLANTILLA), "genapp.validation.required",
                    new String[] { I18NUtils.tradueix(get(NOMPLANTILLA)) }, null);
        }
        ...
        if (str != null) {
            ...
            for (String lang : idiomes) {
                if (prop.getProperty(lang) == null) {
                    result.rejectValue(get(MISSATGE), "custodiainfo.missatge.traduccio", new String[] { lang }, null);
                }
            }
    ...
    public List<StringKeyValue> getReferenceListForPluginID(HttpServletRequest request, ModelAndView mav, Where where)
        ...
        return pluginRefList.getReferenceList(PluginFields.PLUGINID,
                Where.AND(where,
                        Where.OR(PluginFields.ENTITATID.equal(LoginInfo.getInstance().getEntitatID()),
