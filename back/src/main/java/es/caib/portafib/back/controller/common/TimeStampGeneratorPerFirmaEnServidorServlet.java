...
public class TimeStampGeneratorPerFirmaEnServidorServlet extends HttpServlet {
  ...
  protected void processServlet(HttpServletRequest request, HttpServletResponse response,
    ...
    byte[] timeStamp;
    try {
      timeStamp = signaturePlugin.generateTimeStamp(signaturesSetID, signatureIndex,
          inputRequest);

      if (timeStamp == null) {
        String msg = "El generador de TimeStamp per la petició " + signaturesSetID + "["
            + signatureIndex + "] ha retornat null";
        log.error(msg);
      ...
      String msg = "Error greu cridant el TimeStampGenerator per la petició "
          + signaturesSetID + "[" + signatureIndex + "]: " + e.getMessage();
      log.error(msg, e);
      response.sendError(HttpServletResponse.SC_NOT_FOUND);
