...
public class EstadisticaAdenController extends EstadisticaController {
    ...
    @Override
    public EstadisticaFilterForm getEstadisticaFilterForm(Integer pagina, ModelAndView mav, HttpServletRequest request)
        ...
        if (filterForm.isNou()) {
             ...
             * getContextWeb() + "/grafic', true)", AdditionalButtonStyle.INFO));
             */

            filterForm.addAdditionalButtonForEachItem(new AdditionalButton("fas fa-info-circle", "genapp.viewtitle",
        ...
        if (isEstadistiquesPerUsrApp(request)) {
            ...
            filterForm.addAdditionalButton(new AdditionalButton("fas fa-user", "estadistiques.per.persona",
                    getContextWeb() + "/canvipersonaaplicacio/false", AdditionalButtonStyle.INFO));
            ...
            filterForm.addAdditionalButton(new AdditionalButton("fas fa-hdd", "estadistiques.per.aplicacio",
                    getContextWeb() + "/canvipersonaaplicacio/true", AdditionalButtonStyle.INFO));

            filterForm.addHiddenField(USUARIAPLICACIOID);
            filterForm.getHiddenFields().remove(USUARIENTITATID);
        }
        filterForm.addAdditionalButton(new AdditionalButton("fas fa-minus", "estadistiques.simples",
                getContextWeb() + "/search", AdditionalButtonStyle.INFO));
    ...
    public ModelAndView search(HttpServletRequest request, HttpServletResponse response) throws I18NException {
        ...
        filterForm.getAdditionalButtons().clear();
        filterForm.addAdditionalButton(new AdditionalButton("fas fa-plus", "estadistiques.complexes",
                getContextWeb() + "/list", AdditionalButtonStyle.INFO));
    ...
    public void exportList(@PathVariable("dataExporterID") String dataExporterID, HttpServletRequest request,
        ...
        Map<String, Map<String, Double>> agrupades = agruparPerMes(estadistiques, usuarisSet, isUsrApp);

        List<String> usuaris = new ArrayList<String>(usuarisSet);

        Set<String> dates = agrupades.keySet();

        PortaFIBDataExporter dataExporter = (PortaFIBDataExporter) DataExporterManager.getByID(dataExporterID);
    ...
    private Map<String, Map<String, Double>> agruparPerMes(List<Estadistica> list, Set<String> usuaris,
            boolean isUsrApp) {
