...
@SessionAttributes(
        types = { UsuariEntitatFavoritForm.class, UsuariEntitatFavoritFilterForm.class, SeleccioUsuariForm.class })
public class GestioUsuariEntitatFavoritController extends UsuariEntitatFavoritController {

    public static final String USUARI_ENTITAT_ID_HOLDER = "GestioUsuariEntitatFavoritController_USUARI_ENTITAT_ID_HOLDER";
    ...
    @Override
    public UsuariEntitatFavoritForm getUsuariEntitatFavoritForm(UsuariEntitatFavoritJPA _jpa, boolean __isView,
    ...
    @Override
    public void postList(HttpServletRequest request, ModelAndView mav, UsuariEntitatFavoritFilterForm filterForm,
            List<UsuariEntitatFavorit> list) throws I18NException {
        ...
        for (UsuariEntitatJPA ueJPA : fullUsers) {
            ...
            StringKeyValue skv = new StringKeyValue(ueJPA.getUsuariEntitatID(),
                    upJPA.getNom() + " " + upJPA.getLlinatges() + " (" + Utils.ofuscarDNI(upJPA.getNif()) + ")");
            lskv.add(skv);
        }

        // Convertim a map la llista d'UsuariPersona
        Map<String, String> tmp = org.fundaciobit.genapp.common.utils.Utils.listToMap(lskv);
