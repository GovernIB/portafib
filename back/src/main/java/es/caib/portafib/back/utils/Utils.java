...
public class Utils {
        ...
        if (baseFilter.isVisibleOrderBy() && theField != null) {
            ...
            if (orderedBythisfield) {
                ...
                html = "<span style=\"cursor:row-resize\" onClick=\"javascript:executeOrderBy('" + field + "', "
                        ...
                        + getText(code) + "<i class=\"" + (isOrderedAsc ? "fas fa-chevron-up" : "fas fa-chevron-down")
                        + "\"></i></span>";
            } else {
                html = "<span style=\"cursor:row-resize\" onclick=\"javascript:executeOrderBy('" + field
    ...
    public static List<StringKeyValue> usuariEntitatList2StringKeyValue(List<UsuariEntitatJPA> users) {
    ...
    public static void addUserToList(List<StringKeyValue> listNovaFirma, UsuariEntitatJPA usuariEntitat) {
        String carrec = usuariEntitat.getCarrec();
        if (carrec == null) {
            listNovaFirma.add(
                    new StringKeyValue(usuariEntitat.getUsuariEntitatID(), getNom(usuariEntitat.getUsuariPersona())));
        } else {
            listNovaFirma
                    .add(new StringKeyValue(usuariEntitat.getUsuariEntitatID(), "(*) " + usuariEntitat.getCarrec()));
        }
    }

    public static String getNom(UsuariPersona up) {
        return up.getNom() + " " + up.getLlinatges() + " (" + ofuscarDNI(up.getNif()) + ")";
    }

    public static String getLlinatgeNom(UsuariPersona up) {
        return up.getLlinatges() + ", " + up.getNom() + " (" + ofuscarDNI(up.getNif()) + ")";
    ...
    public static String getContentDispositionHeader(boolean attachment, String filename) {
        ...
        return (attachment ? "attachment" : "inline") + "; filename=\"" + filename + "\"" + "; filename*=UTF-8''"
                + utf8filename;
    }

    public static final String ofuscarDNI(String dni) {
        if (dni == null) {
            return null;
        }
        if (dni.length() > 5) {
