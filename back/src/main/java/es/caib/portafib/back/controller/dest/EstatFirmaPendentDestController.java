...
public class EstatFirmaPendentDestController extends EstatFirmaAbstractDestController {
    ...
    public Where getAdditionalCondition(HttpServletRequest request) throws I18NException {
        ...
        if (mostrarPendentDeRevisors()) {
            Where w = Where.AND(super.getAdditionalCondition(request), getWhereNoPendentRevisor());
            SubQuery<EstatDeFirma, Long> subQuery = new SubQuery<EstatDeFirma, Long>(
                    EstatDeFirmaFields.ESTATDEFIRMAID.select, this.estatDeFirmaEjb, w);
            return Where.AND(super.getAdditionalCondition(request), EstatDeFirmaFields.ESTATDEFIRMAID.notIn(subQuery));
        } else {
            return Where.AND(super.getAdditionalCondition(request), getWhereNoPendentRevisor());
        }
    }
