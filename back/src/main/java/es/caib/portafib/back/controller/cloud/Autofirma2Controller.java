package es.caib.portafib.back.controller.cloud;
 ...
 */
@Controller
@RequestMapping(value = Autofirma2Controller.CONTEXTWEB)
@SessionAttributes(types = { AutoFirmaForm.class, FitxerFilterForm.class })
public class Autofirma2Controller extends FitxerController
  implements PeticioDeFirmaFields {

  public static final String CONTEXTWEB = "/develop/signmodule";

  public static final String AUTOFIRMA = "AUTOFIRMA_CLOUD";

  // XYZ Revisar si això fa falta
  @EJB(mappedName = "portafib/AutofirmaEJB/local")
  protected AutofirmaLocal autofirmaEjb;
  
  // XYZ ha de desapareixer
  ...
  @RequestMapping(value = "/firmanuvol")
  public void firmanuvol(
    ...
    byte[] pdf = new CloudSign().doSign();
    
    response.setContentType("application/pdf");

    response.setHeader("Content-disposition","inline; filename=pdf_firmat.pdf");
...
//  }
  ...
  */
  ...
  public ModelAndView autofirmaPost(HttpServletRequest request, HttpServletResponse response,
    ...
    // List<AppletSignFile> appletSignFileList = new ArrayList<AppletSignFile>();
    ...
    /* XYZ
    appletSignFileList.add(new AppletSignFile(source, destination, idname, location_sign_table, reason,
    ...
    */
    ...
    // XYZ
    //appletSignFileList.get(0).setSourceFile(pdfAdaptat);
    
    //AppletConfig appletConfig = Utils.getAppletConfig(entitat, 
    //    langUI, form.isJnlp()? "" : (CONTEXTWEB + "/final/" + id)); // redirect == null
    ...
    final String view = "PluginDeFirmaContenidor_AutoFirma2";
  ...
  public void fitxerAdaptat(@PathVariable("id") Long id,
    ...
    try {
      File fitxerPDF = form.getFitxerAFirmarIDFile();
      dstPDF = getFitxerAdaptatPath(form.getUsuariEntitatID() ,id);
      generaFitxerAdaptat(fitxerPDF, dstPDF, form.getIdioma(), form.getLogoSegell().getFitxerID(),
          form.getAttachments(), (int)form.getPosicioTaulaFirmesID(),
          form.getTitol(), form.getDescripcio());
    } catch (I18NException e) {
      // TODO Enviar missatge
