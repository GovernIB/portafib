 ...
 * 3 abr 2025 9:17:41
 */
public abstract class AbstractPropietatsController extends PropietatGlobalController {
    ...
    public String getSessionAttributeFilterForm() {
        return "Abstract_Properties_" + this.getClass().getName() + "_filterForm";
    }
    
    
    @Override
    public String getEntityNameCode() {
        return "propietat." + getTipusPropietat() + ".title";
    }

    @Override
    public String getEntityNameCodePlural() {
        return "propietat." + getTipusPropietat() + ".title.plural";
    }
    
    

    protected abstract int getTipusPropietat();

    @Override
    public Where getAdditionalCondition(HttpServletRequest request) throws I18NException {

        switch (getTipusPropietat()) {
            case PropietatsConstants.TIPUS_PROPIETAT_BBDD_GLOBAL:
                return ENTITATID.isNull();

            case PropietatsConstants.TIPUS_PROPIETAT_BBDD_PER_ENTITAT:
    ...
    public List<PropietatGlobal> executeSelect(ITableManager<PropietatGlobal, java.lang.Long> ejb, Where where,
        ...
        final int tipus = getTipusPropietat();
        if (tipus == PropietatsConstants.TIPUS_PROPIETAT_FITXER_PORTAFIB_PROPERTIES
                || tipus == PropietatsConstants.TIPUS_PROPIETAT_FITXER_PORTAFIB_SYSTEM_PROPERTIES
                || tipus == PropietatsConstants.TIPUS_PROPIETAT_JAVA) {

            Properties all;
            switch (tipus) {
                case PropietatsConstants.TIPUS_PROPIETAT_FITXER_PORTAFIB_PROPERTIES:
                    all = Configuracio.getPortaFIBProperties();
                break;
                case PropietatsConstants.TIPUS_PROPIETAT_FITXER_PORTAFIB_SYSTEM_PROPERTIES:
                    all = Configuracio.getPortaFIBSystemProperties();
                break;
                default:
                case PropietatsConstants.TIPUS_PROPIETAT_JAVA:
                    all = System.getProperties();
                break;
            }
...
*/
        ...
        if (propietatGlobalFilterForm.isNou()) {
            ...
            switch (tipus) {
                ...
                default:
                    ...
                    if (tipus != PropietatsConstants.TIPUS_PROPIETAT_JAVA) {

                        String fitxer = (tipus == PropietatsConstants.TIPUS_PROPIETAT_FITXER_PORTAFIB_PROPERTIES)
                                ? "portafib.properties"
                                : "portafib.system.properties";

                        propietatGlobalFilterForm.setSubTitleCode(
                                "=Aquí es mostren les propietats del fitxer " + fitxer + ". No es poden modificar.");
                    }
    ...
    public void postList(HttpServletRequest request, ModelAndView mav, PropietatGlobalFilterForm filterForm,
        ...
        switch (getTipusPropietat()) {
            case PropietatsConstants.TIPUS_PROPIETAT_BBDD_GLOBAL:
                propietats = PropietatsConstants.PROPIETATS_BBDD_GLOBALS;
            break;

            case PropietatsConstants.TIPUS_PROPIETAT_BBDD_PER_ENTITAT:
                propietats = PropietatsConstants.PROPIETATS_BBDD_PER_ENTITAT;
            break;
            ...
            default:
                throw new I18NException("genapp.comodi", "No podem gestionar Propietats de Tipus " + getTipusPropietat());
