/*
 * API Interna de PortaFIB que ofereix serveis associats amb la firma digital
 * Conjunt de Serveis REST de PortaFIB per atendre peticions de firma en servidor, firma web síncrona, firma web asíncrona, validació de firma i plantilla de flux de firma.
 *
 * The version of the OpenAPI document: 1.0-SNAPSHOT
 * Contact: firma@fundaciobit.org
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */


package es.caib.portafib.apiinterna.client.signature.v1.model;

import java.util.Objects;
import java.util.Arrays;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonTypeName;
import com.fasterxml.jackson.annotation.JsonValue;
import com.fasterxml.jackson.annotation.JsonPropertyOrder;
import com.fasterxml.jackson.annotation.JsonTypeName;

/**
 * Informació de les validacions realitzades despres de la firma.
 */
@JsonPropertyOrder({
  ValidationInfo.JSON_PROPERTY_CHECK_ADMINISTRATION_I_D_OF_SIGNER,
  ValidationInfo.JSON_PROPERTY_CHECK_DOCUMENT_MODIFICATIONS,
  ValidationInfo.JSON_PROPERTY_CHECK_VALIDATION_SIGNATURE,
  ValidationInfo.JSON_PROPERTY_NO_CHECK_VALIDATION_REASON
})
@javax.annotation.Generated(value = "org.openapitools.codegen.languages.JavaClientCodegen", comments = "Generator version: 7.10.0")
public class ValidationInfo {
  public static final String JSON_PROPERTY_CHECK_ADMINISTRATION_I_D_OF_SIGNER = "checkAdministrationIDOfSigner";
  @javax.annotation.Nullable
  private Boolean checkAdministrationIDOfSigner;

  public static final String JSON_PROPERTY_CHECK_DOCUMENT_MODIFICATIONS = "checkDocumentModifications";
  @javax.annotation.Nullable
  private Boolean checkDocumentModifications;

  public static final String JSON_PROPERTY_CHECK_VALIDATION_SIGNATURE = "checkValidationSignature";
  @javax.annotation.Nullable
  private Boolean checkValidationSignature;

  public static final String JSON_PROPERTY_NO_CHECK_VALIDATION_REASON = "noCheckValidationReason";
  @javax.annotation.Nullable
  private String noCheckValidationReason;

  public ValidationInfo() {
  }

  public ValidationInfo checkAdministrationIDOfSigner(@javax.annotation.Nullable Boolean checkAdministrationIDOfSigner) {
    
    this.checkAdministrationIDOfSigner = checkAdministrationIDOfSigner;
    return this;
  }

  /**
   * S’ha verificat que l’identificador del firmant és la del que estava previst que firmàs. Valor buit indica que no s&#39;ha realitzat la validació.
   * @return checkAdministrationIDOfSigner
   */
  @javax.annotation.Nullable
  @JsonProperty(JSON_PROPERTY_CHECK_ADMINISTRATION_I_D_OF_SIGNER)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)

  public Boolean getCheckAdministrationIDOfSigner() {
    return checkAdministrationIDOfSigner;
  }


  @JsonProperty(JSON_PROPERTY_CHECK_ADMINISTRATION_I_D_OF_SIGNER)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public void setCheckAdministrationIDOfSigner(@javax.annotation.Nullable Boolean checkAdministrationIDOfSigner) {
    this.checkAdministrationIDOfSigner = checkAdministrationIDOfSigner;
  }

  public ValidationInfo checkDocumentModifications(@javax.annotation.Nullable Boolean checkDocumentModifications) {
    
    this.checkDocumentModifications = checkDocumentModifications;
    return this;
  }

  /**
   * S’ha verificat que no s’hagi modificat el document original. Valor buit indica que no s&#39;ha realitzat la validació.
   * @return checkDocumentModifications
   */
  @javax.annotation.Nullable
  @JsonProperty(JSON_PROPERTY_CHECK_DOCUMENT_MODIFICATIONS)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)

  public Boolean getCheckDocumentModifications() {
    return checkDocumentModifications;
  }


  @JsonProperty(JSON_PROPERTY_CHECK_DOCUMENT_MODIFICATIONS)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public void setCheckDocumentModifications(@javax.annotation.Nullable Boolean checkDocumentModifications) {
    this.checkDocumentModifications = checkDocumentModifications;
  }

  public ValidationInfo checkValidationSignature(@javax.annotation.Nullable Boolean checkValidationSignature) {
    
    this.checkValidationSignature = checkValidationSignature;
    return this;
  }

  /**
   * S’ha verificat que la firma és correcte. Valor buit indica que no s&#39;ha realitzat la validació.
   * @return checkValidationSignature
   */
  @javax.annotation.Nullable
  @JsonProperty(JSON_PROPERTY_CHECK_VALIDATION_SIGNATURE)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)

  public Boolean getCheckValidationSignature() {
    return checkValidationSignature;
  }


  @JsonProperty(JSON_PROPERTY_CHECK_VALIDATION_SIGNATURE)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public void setCheckValidationSignature(@javax.annotation.Nullable Boolean checkValidationSignature) {
    this.checkValidationSignature = checkValidationSignature;
  }

  public ValidationInfo noCheckValidationReason(@javax.annotation.Nullable String noCheckValidationReason) {
    
    this.noCheckValidationReason = noCheckValidationReason;
    return this;
  }

  /**
   * Només s&#39;omple si checkValidationSignature val false
   * @return noCheckValidationReason
   */
  @javax.annotation.Nullable
  @JsonProperty(JSON_PROPERTY_NO_CHECK_VALIDATION_REASON)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)

  public String getNoCheckValidationReason() {
    return noCheckValidationReason;
  }


  @JsonProperty(JSON_PROPERTY_NO_CHECK_VALIDATION_REASON)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public void setNoCheckValidationReason(@javax.annotation.Nullable String noCheckValidationReason) {
    this.noCheckValidationReason = noCheckValidationReason;
  }

  @Override
  public boolean equals(Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    ValidationInfo validationInfo = (ValidationInfo) o;
    return Objects.equals(this.checkAdministrationIDOfSigner, validationInfo.checkAdministrationIDOfSigner) &&
        Objects.equals(this.checkDocumentModifications, validationInfo.checkDocumentModifications) &&
        Objects.equals(this.checkValidationSignature, validationInfo.checkValidationSignature) &&
        Objects.equals(this.noCheckValidationReason, validationInfo.noCheckValidationReason);
  }

  @Override
  public int hashCode() {
    return Objects.hash(checkAdministrationIDOfSigner, checkDocumentModifications, checkValidationSignature, noCheckValidationReason);
  }

  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class ValidationInfo {\n");
    sb.append("    checkAdministrationIDOfSigner: ").append(toIndentedString(checkAdministrationIDOfSigner)).append("\n");
    sb.append("    checkDocumentModifications: ").append(toIndentedString(checkDocumentModifications)).append("\n");
    sb.append("    checkValidationSignature: ").append(toIndentedString(checkValidationSignature)).append("\n");
    sb.append("    noCheckValidationReason: ").append(toIndentedString(noCheckValidationReason)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }

}

