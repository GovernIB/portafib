 ...
 *
 */
public abstract class AbstractMiniAppletSignatureWebPlugin extends AbstractPluginProperties implements
    ISignatureWebPlugin {
 ...
 /// XYZ No ho necessit public static final String BASE_DIR = MINIAPPLETASAPPLET_BASE_PROPERTIES + "base_dir";
   ...
   * 
   */
  public AbstractMiniAppletSignatureWebPlugin() {
    super();
  }
   ...
   * @param properties
   */
  public AbstractMiniAppletSignatureWebPlugin(String propertyKeyBase, Properties properties) {
   ...
   * @param propertyKeyBase
   */
  public AbstractMiniAppletSignatureWebPlugin(String propertyKeyBase) {
  ...
  public void requestGET(String pluginRequestPath, String relativePath, String signaturesSetID,
    ...
    // XYZ
    log.info("======== REQUEST GET " + getSimpleName() + " =========== ");
    ...
    if (relativePath.endsWith(SHOW_MINIAPPLET_PAGE)) {
      ...
      log.error("======== REQUEST GET APPLET DESCONEGUT =========== ");
  ...
  public void requestPOST(String pluginRequestPath, String relativePath,
    ...
    if (relativePath.endsWith(DESTINATION_DOC_PAGE)) { 
      ...
      rubricPage(pluginRequestPath, relativePath, signaturesSet,
          signatureIndex, request, response, uploadedFiles);
    } if (relativePath.endsWith(ERROR_PAGE)) { 
      errorPage(pluginRequestPath, relativePath, signaturesSet,
          signatureIndex, request, response,  uploadedFiles);
    } else {
    
      log.error("======== REQUEST POST APPLET DESCONEGUT =========== ");
      log.error("pluginRequestPath: " + pluginRequestPath);
      log.error("relativePath: " + relativePath);
      log.error("signatureID: " + signaturesSetID);
      log.error("signatureIndex: " + signatureIndex);
...
//---------------------------------------------------------------------------
 ...
 private void errorPage(String pluginRequestPath, String relativePath, SignaturesSet signaturesSet,
   ...
   try {
     ...
     if (uploadedFiles.size() == 0) {
       ...
       // TODO XYZ Traduir emprant langUI
       status.setErrorMsg("S'ha rebut un error però aquest no conté detalls del tipus d'error que s'ha produït");
    ...
    if (quefer == null) {
      ...
      if (content == null) {
        ...
        try {
          ...
          // XYZ File localJS = new File(session.getServletContext().getRealPath(localPath));
  ...
  */
  ...
  protected void generateHeader(HttpServletRequest request, String pluginRequestPath,
    ...
    out.println("<title>MiniAppletAsAppletPlugin</title>");
  ...
  protected String getTraduccio(String key, Locale locale) {
    ...
    try {
       ...
       ResourceBundle rb = ResourceBundle.getBundle("miniappletasapplet", locale /*, utf8Control*/);
