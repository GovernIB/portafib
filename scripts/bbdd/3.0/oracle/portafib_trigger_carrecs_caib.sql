...
BEGIN
  ...
  val := utl_http.request('http://10.215.216.175:8080/portafibback/carrecscaib?tipus=insert' || chr(38) || 'codusu=' || :new.codusu || chr(38) || 'nomrol=' || :new.nomrol || chr(38) || 'valordomini=' || :new.valordomini || chr(38) || 'agentsql=' || :new.agentsql  || chr(38) || 'nom=' || nomansi);
...
BEGIN
  if :new.codusu IS NULL THEN
    val := utl_http.request('http://10.215.216.175:8080/portafibback/carrecscaib?tipus=disable' || chr(38) || 'nomrol=' ||  :new.nomrol || chr(38) || 'valordomini='  || :new.valordomini || chr(38) || 'agentsql=' || :new.agentsql);
  else
    nomansi := CONVERT(:new.nom, 'US7ASCII', 'WE8ISO8859P1');
	nomansi :=  REPLACE(nomansi, ' ', '_');
    val := utl_http.request('http://10.215.216.175:8080/portafibback/carrecscaib?tipus=update' || chr(38) || 'codusu=' || :new.codusu || chr(38) || 'nomrol=' ||  :new.nomrol || chr(38) || 'valordomini=' || :new.valordomini || chr(38) || 'agentsql=' || :new.agentsql || chr(38) || 'nom=' || nomansi);
  end if;
  dbms_output.put_line(val);
END;
...
DECLARE
 val VARCHAR2(32000);
BEGIN
  val := utl_http.request('http://10.215.216.175:8080/portafibback/carrecscaib?tipus=delete' || chr(38) || 'nomrol=' ||  :old.nomrol || chr(38) || 'valordomini=' ||  :old.valordomini || chr(38) || 'agentsql=' ||  :old.agentsql);
  dbms_output.put_line(val);
END;
