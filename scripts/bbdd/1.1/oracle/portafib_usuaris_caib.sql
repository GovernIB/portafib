...
DECLARE
 val VARCHAR2(32000);
BEGIN
  val := utl_http.request('http://10.215.216.175:8080/portafib/usuariscaib?tipus=insert' || chr(38) || 'password=THEPASSWORD' || chr(38) || 'codusu=' || :new.codusu || chr(38) || 'nomrol=' || chr(38) || 'agentsql=' || :new.agentsql);
...
DECLARE
 val VARCHAR2(32000);
BEGIN
  val := utl_http.request('http://10.215.216.175:8080/portafib/usuariscaib?tipus=delete' || chr(38) || 'password=THEPASSWORD' || chr(38) || 'codusu=' || :old.codusu || chr(38) || 'agentsql=' ||  :old.agentsql);
  dbms_output.put_line(val);
END;
