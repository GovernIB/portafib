...
public class FluxDeFirmesLogicaEJB extends FluxDeFirmesEJB implements FluxDeFirmesLogicaLocal, FluxDeFirmesFields {
    ...
    public FluxDeFirmesJPA createFull(FluxDeFirmesJPA flux) throws I18NException, I18NValidationException {
        ...
        if ((blocs == null) || (blocs.size() == 0)) {
            throw new I18NException("genapp.validation.required", new I18NArgumentCode("blocDeFirmes.blocDeFirmes"));
        }
        for (BlocDeFirmesJPA blocDeFirmesJPA : blocs) {
            Set<FirmaJPA> firmes = blocDeFirmesJPA.getFirmas();
            if ((firmes == null) || (firmes.size() == 0)) {
                throw new I18NException("genapp.validation.required", new I18NArgumentCode("firma.firma.plural"));
            }
        }
    ...
    public void validatePlantillaFluxDeFirmes(FluxDeFirmesJPA flux, final boolean isNou, PlantillaFluxDeFirmes pff)
        ...
        if (pff != null) {
            ...
            if (number == null || number != 0) {
                ...
                I18NFieldError fieldError = new I18NFieldError(FluxDeFirmesFields.NOM,
                        new I18NTranslation("genapp.validation.unique", new I18NArgumentString(flux.getNom()),
                ...
                for (BlocDeFirmesJPA blocDeFirmesJPA : blocs) {
                    ...
                    for (FirmaJPA firmaJPA : firmes) {
                        ...
                        if (ue == null) {
                            ...
                            if (ue == null) {
                                log.error("No es pot inicialitzar l'usuariEntitat " + firmaJPA.getDestinatariID()
                                        + " de la firma " + firmaJPA.getFirmaID(), new Exception());
                            } else {
                        ...
                        if (ue != null) {
                            ...
                            try {
                                ...
                                UsuariEntitatJPA ueJPA = UsuariEntitatLogicaEJB.findByPrimaryKeyFull(usuariEntitatEjb,
                                        ue.getUsuariEntitatID());
...
//            entityManager.clear();
    ...
    public Set<Long> deleteFull(Long fluxDeFirmesID) throws I18NException {
        ...
        permisUsuariPlantillaEjb.delete(PermisUsuariPlantillaFields.PLANTILLAFLUXDEFIRMESID.equal(fluxDeFirmesID));

        // Esborrar Permisos grups
        permisGrupPlantillaEjb.delete(PermisGrupPlantillaFields.PLANTILLAFLUXDEFIRMESID.equal(fluxDeFirmesID));

        // Esborrar Plantilla
        plantillaFluxDeFirmesLogicaEjb.delete(PlantillaFluxDeFirmesFields.FLUXDEFIRMESID.equal(fluxDeFirmesID));
        ...
        if (pff != null) {
            ...
            if (pff.getCompartir() != null) {
                ...
                permisUsuariPlantillaEjb
                        .delete(PermisUsuariPlantillaFields.PLANTILLAFLUXDEFIRMESID.equal(pff.getFluxDeFirmesID()));

                // grups
                permisGrupPlantillaEjb
                        .delete(PermisGrupPlantillaFields.PLANTILLAFLUXDEFIRMESID.equal(pff.getFluxDeFirmesID()));
            }
    ...
    @Override
    public Set<Long> deleteFullPlantillaFluxDeFirmesUsuari(long fluxDeFirmesID) throws I18NException {
    ...
    @Override
    public boolean afegirFirmaABloc(UsuariEntitatJPA usuariEntitat, UsuariExtern usuariExtern, BlocDeFirmesJPA bloc)
