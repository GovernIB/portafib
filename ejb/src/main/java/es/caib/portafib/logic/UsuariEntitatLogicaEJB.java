...
import es.caib.portafib.persistence.validator.UsuariEntitatBeanValidator;
...
public class UsuariEntitatLogicaEJB extends UsuariEntitatEJB implements UsuariEntitatLogicaLocal, ConstantsV2 {
    ...
    protected RebreAvisLogicaLocal rebreAvisEjb;
    ...
    private es.caib.portafib.ejb.IdiomaService idiomaEjb;
    ...
    protected PropietatGlobalLogicaLocal propietatGlobalEjb;
    ...
    public static UsuariEntitatJPA findByPrimaryKeyFull(UsuariEntitatService usuariEntitatEjb, String usuariEntitatID) {
        UsuariEntitatJPA ue;
    ...
    public UsuariEntitatJPA create(UsuariPersonaJPA usuariPersonaJPA, UsuariEntitatJPA usuariEntitatJPA,
            Set<String> virtualRoles) throws I18NException, I18NValidationException, Exception {
        ...
        UsuariPersonaBeanValidator upbv = new UsuariPersonaBeanValidator(validadorUP, idiomaEjb,
                this.usuariPersonaLogicaEjb);
    ...
    @Override
    public UsuariEntitatJPA create(final String usuariPersonaID, UsuariEntitatJPA usuariEntitatJPA,
            Set<String> virtualRoles) throws I18NValidationException, I18NException {
        final boolean isNou = true;
        if (usuariEntitatJPA == null) {
            ...
            final String entitatID = usuariEntitatJPA.getEntitatID();
            usuariEntitatJPA.setUsuariPersonaID(usuariPersonaID);
            usuariEntitatJPA.setUsuariEntitatID(entitatID + "_" + usuariPersonaID);

            UsuariEntitatLogicValidator<UsuariEntitatJPA> validadorUE = new UsuariEntitatLogicValidator<UsuariEntitatJPA>();
            UsuariEntitatBeanValidator uebv = new UsuariEntitatBeanValidator(validadorUE, this.custodiaInfoEjb,
                    entitatEjb, this, this.usuariPersonaLogicaEjb);
            ...
            // 2.5.- Alta de Certes Notificacions si hi ha la propietat XXXX
            Boolean addnotificationstonewuser = propietatGlobalEjb.getBooleanPropertyByEntitat(entitatID,
                    "es.caib.portafib.addnotificationstonewuser");

            if (addnotificationstonewuser != null && addnotificationstonewuser) {
                // Afegir notificacions a l'usuari
                rebreAvisEjb.create(usuariEntitatJPA.getUsuariEntitatID(),
                        ConstantsV2.NOTIFICACIOAVIS_REQUERIT_PER_VALIDAR, true);
                rebreAvisEjb.create(usuariEntitatJPA.getUsuariEntitatID(),
                        ConstantsV2.NOTIFICACIOAVIS_REQUERIT_PER_FIRMAR, true);
                rebreAvisEjb.create(usuariEntitatJPA.getUsuariEntitatID(),
                        ConstantsV2.NOTIFICACIOAVIS_REQUERIT_PER_REVISAR, true);
            }
        }
        return usuariEntitatJPA;
    }
    ...
    public Set<Long> deleteFull(String usuariEntitatID) throws I18NException {
        ...
        if (numFirmes == null || numFirmes != 0) {
            ...
            if (carrec == null) {
                ...
                throw new I18NException("usuariEntitat.error.borrar.firmespendents", new I18NArgumentString(nom));
            } else {
        ...
        if (usuariEntitatJPA.getCarrec() == null) {
            ...
            Where wCarrecs = Where.AND(CARREC.isNotNull(), USUARIPERSONAID.equal(usuariEntitatJPA.getUsuariPersonaID()),
                    ENTITATID.equal(usuariEntitatJPA.getEntitatID()));

            List<String> carrecs = this.executeQuery(CARREC, wCarrecs);
            if (carrecs.size() != 0) {
                throw new I18NException("usuariEntitat.error.borrar.carrecs", Arrays.toString(carrecs.toArray()));
            }

            // (3) L'usuari té Peticions de Firma en marxa
            List<PeticioDeFirma> peticions;
            // Peticions de Firma en marxa o pasades
            Where w1 = PeticioDeFirmaFields.TIPUSESTATPETICIODEFIRMAID.in(new Integer[] {
                    ConstantsV2.TIPUSESTATPETICIODEFIRMA_ENPROCES, ConstantsV2.TIPUSESTATPETICIODEFIRMA_PAUSAT });
            ...
            if (peticions != null && peticions.size() != 0) {
                ...
                throw new I18NException("usuariEntitat.error.borrar.peticionspendents", usuariEntitatID,
                        Arrays.toString(peticionsStr.toArray()));
            }
            ...
            // Esborrar favorits
            usuariEntitatFavoritEjb.delete(Where.OR(UsuariEntitatFavoritFields.ORIGENID.equal(usuariEntitatID),
                    UsuariEntitatFavoritFields.FAVORITID.equal(usuariEntitatID)));
            ...
            List<Long> plantilles = plantillaFluxDeFirmesEjb.executeQuery(PlantillaFluxDeFirmesFields.FLUXDEFIRMESID,
            ...
            List<Long> peticionsID = this.peticioDeFirmaLogicaEjb.executeQuery(PeticioDeFirmaFields.PETICIODEFIRMAID,
                    w2);

            for (Long peticioDeFirmaID : peticionsID) {
                fitxers.addAll(this.peticioDeFirmaLogicaEjb.deleteFullUsingUsuariEntitat(peticioDeFirmaID,
                        usuariEntitatJPA.getUsuariPersonaID()));
            }
     ...
     * 
     */
    public List<UsuariEntitatJPA> selectFavorits(String usuariEntitatID, String roleID, boolean incloureCarrecs)
        ...
        Where favorits;
        {
            ...
            if (roleID != null) {
                whereRole = UsuariEntitatFavoritFields.FAVORITID.in(roleUsuariEntitatLogicaEjb.getSubQuery(
                        RoleUsuariEntitatFields.USUARIENTITATID, RoleUsuariEntitatFields.ROLEID.equal(roleID)));
            }
            ...
            favoritsSQ = usuariEntitatFavoritEjb.getSubQuery(UsuariEntitatFavoritFields.FAVORITID,
                    Where.AND(UsuariEntitatFavoritFields.ORIGENID.equal(usuariEntitatID), whereRole));
        ...
        if (incloureCarrecs) {
            ...
            carrecs = Where.AND(UsuariEntitatFields.ENTITATID.equal(ue.getEntitatID()),
                    UsuariEntitatFields.ACTIU.equal(true), UsuariEntitatFields.CARREC.isNotNull());
        }
     ...
     */
    @Override
    public UsuariEntitatJPA createFull(UsuariEntitatJPA usuariEntitat) throws I18NValidationException, I18NException {

        if (usuariEntitat.getUsuariEntitatID() == null) {
            if (usuariEntitat.getEntitatID() != null && usuariEntitat.getUsuariPersonaID() != null) {
                usuariEntitat
                        .setUsuariEntitatID(usuariEntitat.getEntitatID() + "_" + usuariEntitat.getUsuariPersonaID());
            }
        }

        UsuariEntitatBeanValidator beanValidator;
        beanValidator = new UsuariEntitatBeanValidator(validator, this.custodiaInfoEjb, entitatEjb, this,
                usuariPersonaLogicaEjb);
    ...
    @Override
    public UsuariEntitatJPA createUsuariEntitatExtern(UsuariEntitatJPA usuariEntitatExtern, String entitatID)
...

        {
            ...
            if (usuariPersonaExternaJPA == null) {
                ...
                UsuariPersonaBeanValidator upbv = new UsuariPersonaBeanValidator(validador, idiomaEjb,
                        usuariPersonaLogicaEjb);
                final boolean isNou = true;
        ...
        usuariEntitatExtern.setUsuariEntitatID(entitatID + "_" + usuariPersonaExternaID);
    ...
    @Override
    public UsuariEntitatJPA updateFull(UsuariEntitatJPA usuariEntitat) throws I18NValidationException, I18NException {
    ...
    @Override
    public UsuariEntitatJPA findUsuariEntitatByUsername(String entitatID, String username) throws I18NException {
        ...
        List<UsuariEntitat> list = select(
                Where.AND(USUARIPERSONAID.equal(username), ENTITATID.equal(entitatID), CARREC.isNull()));
    ...
    @Override
    public UsuariEntitatJPA findUsuariEntitatInternByNif(String entitatID, String nif) throws I18NException {
        final boolean isIntern = true;
        return findUsuariEntitatByNif(entitatID, nif, isIntern);
    }

    @Override
    public UsuariEntitatJPA findUsuariEntitatExternByNif(String entitatID, String nif) throws I18NException {
    ...
    public UsuariPersonaJPA findUsuariPersonaExternaByNif(String nif) throws I18NException {
        ...
        List<UsuariPersona> list = usuariPersonaLogicaEjb
                .select(Where.AND(UsuariPersonaFields.NIF.equal(nif), UsuariPersonaFields.USUARIINTERN.equal(false)));
    ...
    protected UsuariEntitatJPA findUsuariEntitatByNif(String entitatID, String nif, boolean isIntern)
        ...
        if (nif == null || nif.trim().length() == 0) {
            throw new I18NException("genapp.validation.required", new I18NArgumentCode("nif"));
        }
        ...
        List<UsuariEntitat> list = select(Where.AND(nifField.equal(nif), usuariInternField.equal(isIntern),
                ENTITATID.equal(entitatID), CARREC.isNull()));
    ...
    protected void activar(String usuariEntitatID, boolean isCarrec) throws I18NException {
        ...
        if (usuariEntitatJPA.getCarrec() != null) {
            ...
            Boolean actiuUsuariEntitatReal = this.executeQueryOne(UsuariEntitatFields.ACTIU,
                    Where.AND(UsuariEntitatFields.USUARIPERSONAID.equal(usuariEntitatJPA.getUsuariPersonaID()),
        ...
        if (usuariEntitatJPA.getCarrec() == null) {
            List<UsuariEntitat> carrecs = this
                    .select(Where.AND(UsuariEntitatFields.USUARIPERSONAID.equal(usuariEntitatJPA.getUsuariPersonaID()),
                            UsuariEntitatFields.ENTITATID.equal(usuariEntitatJPA.getEntitatID()),
    ...
    protected void desactivar(String usuariEntitatID, boolean isCarrec) throws I18NException {
        ...
        IntegerField TIPUSESTATPETICIODEFIRMAID = new FirmaQueryPath().BLOCDEFIRMES().FLUXDEFIRMES().PETICIODEFIRMA()
                .TIPUSESTATPETICIODEFIRMAID();

        Long count = firmaEjb.count(Where.AND(FirmaFields.DESTINATARIID.equal(usuariEntitatID),
                FirmaFields.TIPUSESTATDEFIRMAFINALID.isNull(), TIPUSESTATPETICIODEFIRMAID.in(new Integer[] {
                        ConstantsV2.TIPUSESTATPETICIODEFIRMA_ENPROCES, ConstantsV2.TIPUSESTATPETICIODEFIRMA_PAUSAT })));
        ...
        if (carrec == null) {
            ...
            List<String> carrecsActius = this.executeQuery(UsuariEntitatFields.CARREC,
                    Where.AND(UsuariEntitatFields.USUARIPERSONAID.equal(usuariEntitatJPA.getUsuariPersonaID()),
                            UsuariEntitatFields.ENTITATID.equal(usuariEntitatJPA.getEntitatID()),
                            UsuariEntitatFields.CARREC.isNotNull(), UsuariEntitatFields.ACTIU.equal(true)));
            ...
            if (carrecsActius.size() != 0) {
                ...
                throw new I18NException("usuariEntitat.error.desactivar.abansborrarcarrecs",
                        up.getNom() + " " + up.getLlinatges(), Arrays.toString(carrecsActius.toArray()));
            }
        }
    ...
    private UsuariEntitatJPA checkBasic(String usuariEntitatID, boolean isCarrec) throws I18NException {

        if (usuariEntitatID == null) {
            // error.notfound=No s´ha trobat cap {0} amb {1} igual a {2}
            throw new I18NException("error.notfound", new I18NArgumentCode(isCarrec ? "carrec" : _TABLE_TRANSLATION),
    ...
    public Set<String> getUsuariEntitatIdCurrentUser() {
        ...
        try {
            List<String> listID = executeQuery(USUARIENTITATID,
                    Where.AND(USUARIPERSONAID.equal(username), ACTIU.equal(true), CARREC.isNull()));
            setID.addAll(listID);
