            ...
            for (int i = 0; i < pss.getFileInfoSignatureArray().length; i++) {
                ...
                String signType = pfis.getSignType();
                {
                    ...
                    if (isEmpty(signType)) {
                        ...
                        List<String> list = Arrays
                                .asList(passarelaDeFirmaEjb.getSupportedSignatureTypes(entitatID, null, null));
                        if (list.size() == 0) {
                            // No hi ha plugins disponibles
                            __vr.rejectValue(SIGNTYPE, "plugin.signatureserver.none",
                                    new I18NArgumentString(entitatID));
                        } else {
                            ...
                            if (!list.contains(signType)) {
                                ...
                                __vr.rejectValue(SIGNTYPE, "error.passarela.field.fixedvalues",
                                        new I18NArgumentString(get(SIGNTYPE)),
                                        new I18NArgumentString(Arrays.toString(passarelaDeFirmaEjb
                                                .getSupportedSignatureTypes(entitatID, null, null))));
                            } else {
                                signTypeOK = true;
                            }
                        }
                ...
                if (signTypeOK) {
                    ...
                    if (isEmpty(signType)) {
                        ...
                        if (!Arrays.asList(
                                passarelaDeFirmaEjb.getSupportedSignatureAlgorithms(signType, entitatID, null, null))
                                .contains(signAlgo)) {
                            __vr.rejectValue(SIGNALGO, "error.passarela.field.fixedvalues",
                                    new I18NArgumentString(get(SIGNALGO)),
                                    new I18NArgumentString(Arrays.toString(passarelaDeFirmaEjb
                                            .getSupportedSignatureAlgorithms(signType, entitatID, null, null))));
                        }
                    }
                }
