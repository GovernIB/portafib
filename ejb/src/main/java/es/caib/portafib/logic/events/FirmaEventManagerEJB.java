...
public class FirmaEventManagerEJB implements FirmaEventManagerLocal, ConstantsV2, UsuariEntitatFields {
        ...
        for (FirmaEvent firmaEvent : list) {
            ...
            switch (eventID) {
                ...
                case (int) NOTIFICACIOAVIS_FIRMA_PARCIAL: {
                    ...
                    if (debug) {
                        log.debug(
                                "NOTIFICACIOAVIS_FIRMA_PARCIAL => Destinatari = " + destinatari + " (" + actorID + ")");
                    }
                ...
                case (int) NOTIFICACIOAVIS_PETICIO_PAUSADA: {
                    ...
                    if (soli != null) {
                        ...
                        if (ue != null) {
                            ...
                            firmaEvent
                                    .setDestinatariUsuariAplicacioID(soli.getUsuariAplicacio().getUsuariAplicacioID());
                            filteredlist.add(firmaEvent);
                            if (debug) {
                                log.debug("      SOLICITANT II: UA -> "
                                        + soli.getUsuariAplicacio().getUsuariAplicacioID());
                            }
                        }
                    }
                    ...
                    if (soli == null || ue != null) {
                        // ============== AVIS WEB ?
                        if (eventID == NOTIFICACIOAVIS_PETICIO_REBUTJADA
                                || eventID == NOTIFICACIOAVIS_PETICIO_FIRMADA) {
                ...
                case (int) NOTIFICACIOAVIS_INVALIDAT:
                    ...
                    //EstatDeFirma ef = firmaEvent.getEstatDeFirma();
                    List<EstatDeFirma> destdeleDeFirma = estatDeFirmaEjb
                            .select(Where.AND(EstatDeFirmaFields.FIRMAID.equal(firmaEvent.getFirmaID()),
                ...
                switch ((int) eventID) {
                    ...
                    case (int) NOTIFICACIOAVIS_INVALIDAT:
                        if (firmaEvent.getDestinatariUsuariAplicacioID() != null) {
                            ...
                            avisos.add(crearEmail(firmaEvent, eventCode, firmaEvent.getEstatDeFirmaDescripcio()));
                        }
                    break;
    ...
    protected EmailInfo crearEmail(FirmaEvent event, String subjectCode, String argDesc) throws Exception {

        // Cerca el destinatari del correu o notificacio
        UsuariEntitatJPA ue = UsuariEntitatLogicaEJB.findByPrimaryKeyFull(usuariEntitatEjb,
                event.getDestinatariUsuariEntitatID());
    ...
    protected Solicitant getSolicitant(FirmaEvent firmaEvent, int tipusNotificacio) throws I18NException {
        ...
        if (usuariEntitatID != null) {
            ...
            ua = UsuariAplicacioLogicaEJB.findByPrimaryKeyFull(usuariAplicacioEjb,
                    firmaEvent.getPeticioDeFirmaUsuariAplicacioID());
    ...
    protected DestinatariOfPeticioDeFirma getDestinatariOfPeticioDeFirma(FirmaEvent firmaEvent) throws I18NException {
        ...
        if (firmaEvent.getEstatDeFirmaColaboracioDelegacioID() == null) {
            ...
            where = Where.AND(USUARIENTITATID.equal(firmaEvent.getEstatDeFirmaUsuariEntitatID()),
                    getWhereDeRebreAvis(eventID));

        } else {
            // Hem de cercar el destinatari d'aquesta firma

            where = Where.AND(getWhereDeRebreAvis(eventID),
                    USUARIENTITATID.in(estatDeFirmaEjb.getSubQuery(EstatDeFirmaFields.USUARIENTITATID,
                            Where.AND(EstatDeFirmaFields.COLABORACIODELEGACIOID.isNull(),
     ...
     */
    private void avisWebDePeticioUsuariEntitatFinalitzada(Long peticioDeFirmaID) throws I18NException {
