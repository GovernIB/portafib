...
public class NotificacionsCallBackTimerEJB implements NotificacionsCallBackTimerLocal {
    ...
    private void executeTask(boolean wakeUp) {
        try {
            ...
            List<NotificacioWS> notificacions = notificacioEjb.select(where, 0, maximSeleccionats,
                    new OrderBy(NotificacioWSFields.DATACREACIO), new OrderBy(
                            NotificacioWSFields.REINTENTS)); /* Dins la mateixa data no caldria ordenar pels que
            ...
            for (NotificacioWS notificacioWS : notificacions) {
                ...
                if ((System.currentTimeMillis() - now) > maxTempsNotificant) {
                    log.warn("executeTask: Fa més de " + maxTempsNotificant
                            + " ms que feim Notificacions. Aturam per evitar solapar execucions");
                    break;
                }
            }
    ...
    private void processNotificacio(NotificacioWSJPA notificacioJPA) {
        ...
        try {
            ...
            if (fe == null) {
                // TODO enviar a admin
                log.error("processNotificacio: La notificacio amb ID " + notificacioJPA.getNotificacioID()
            ...
            if (usuariAplicacio == null) {
                // TODO enviar a admin
                log.warn("processNotificacio: No es troba UsuariAplicacio " + usuariAplicacioID
                ...
                if (sender != null) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            bitacolaLogicaEjb.createBitacola(usuariAplicacio.getEntitatID(),
                                    notificacioJPA.getPeticioDeFirmaID(), BITACOLA_TIPUS_PETICIO,
                                    BITACOLA_OP_ERROR_NOTIFICACIO, "Error realitzant notificació: " + cause,
                ...
                int operacio = NotificacioUtils
                        .getOpericioBitacolaForEventType(notificacioInfo.getFirmaEvent().getEventID());
                if (operacio != -1) {
                    bitacolaLogicaEjb.createBitacola(usuariAplicacio.getEntitatID(),
                            notificacioJPA.getPeticioDeFirmaID(), BITACOLA_TIPUS_PETICIO, operacio,
                            "Notificat Usuari-Aplicació: " + usuariAplicacio.getUsuariAplicacioID(),
                            notificacioInfo.getFirmaEvent());
                } else {
                    log.warn("EventID no és correspon a cap operació de bitàcola: "
                            + notificacioInfo.getFirmaEvent().getEventID());
                }
            }
            ...
            if (th instanceof I18NException) {
                Locale loc = new Locale(
                        usuariAplicacio == null ? Configuracio.getDefaultLanguage() : usuariAplicacio.getIdiomaID());
                ...
                if (sendMail != null && sendMail == notificacioJPA.getReintents()) {
                    ...
                    if (usuariAplicacio == null) {
                        ...
                        try {
                            ...
                            log.error("processNotificacio: Error enviant correu a administrador d'entitat " + dest
                                    + " a causa d'errors en la notificació de l'usuari app "
