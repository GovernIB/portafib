...
public class PassarelaDeFirmaWebEJB extends AbstractPassarelaDeFirmaEJB<ISignatureWebPlugin>
    ...
    protected CodiBarresService codiBarresEjb;

    @EJB(mappedName = EstadisticaService.JNDI_NAME, beanName = "EstadisticaEJB")
    ...
    public String startTransaction(PassarelaSignaturesSet signaturesSet, String entitatID, boolean fullView,
        ...
        // Validar
        SignaturesSetBeanValidator ssbv = new SignaturesSetBeanValidator(validator, this, entitatID);
        ...
        try {
            ...
            CustodiaInfo custodiaInfo = custodiaInfoLogicaEjb.getCustodiaUA(usuariAplicacio, null, nom, entitatJPA);
            ...
            // Guardar
            storeSignaturesSet(new PassarelaSignaturesSetWebInternalUse(entitatID, originalNumberOfSignsArray, fullView,
                    signaturesSet, usuariAplicacio.getUsuariAplicacioID(), urlBase, perfilDeFirma, configBySignID,
                    custodiaBySignID, peticioDeFirmaBySignID));

            bitacolaLogicaEjb.createBitacola(InfoBitacola.builder(entitatID).objecteid(signaturesSetID)
                    .tipusObjecte(ConstantsV2.BITACOLA_TIPUS_FIRMASINCRONA)
                    .tipusOperacio(ConstantsV2.BITACOLA_OP_INICIAR).objecte(signaturesSet)
                    .principal(context.getCallerPrincipal()).build());
    ...
    protected PeticioDeFirmaJPA convertPassarelaFileInfoSignature2PeticioDeFirma(String titol, int origenPeticioDeFirma,
            long tipusDocumentID, UsuariAplicacioJPA usuariAplicacio, PassarelaFileInfoSignature pfis,
            CustodiaInfo custodiaInfo, UsuariAplicacioConfiguracioJPA config, EntitatJPA entitatJPA)
        ...
        PeticioDeFirmaJPA peticio = new PeticioDeFirmaJPA(peticioDeFirmaID, titol, descripcio, motiu, fitxerAFirmarID,
                ...
                avisWeb, segellatDeTemps, origenPeticioDeFirma, configuracioDeFirmaID);

        return peticio;
    }

    public PassarelaSecureVerificationCodeStampInfo getCustodiaOfUsuariAplicacio(CustodiaInfo custodiaInfo,
            final UsuariAplicacioJPA usuariAplicacio, EntitatJPA entitatJPA)
        ...
        secureVerificationCodeStampInfo.setBarCodePosition((int) custodiaInfo.getCodiBarresPosicioPaginaID());
    ...
    protected PassarelaSignatureResult convertToPassarelaSignatureResult(boolean addFiles,
            PassarelaSignatureStatusWebInternalUse ss, PassarelaFileInfoSignature pfis, CustodiaInfoJPA custInfo) {
        ...
        if (infoValidacio != null) {
            pvi = new PassarelaValidationInfo(infoValidacio.getCheckAdministrationIDOfSigner(),
                    infoValidacio.getCheckDocumentModifications(), infoValidacio.getCheckValidationSignature(), null);
        }
        ...
        if (custInfo != null) {
            ...
            pCustodyInfo = new PassarelaCustodyInfo(custodyFileID, custodyFileURL, custodyFileCSV,
                    custodyFileCSVValidationWeb, custodyFileCSVGenerationDefinition, custodyFileOriginalFileDirectURL,
    ...
    public PassarelaSignaturesSetWebInternalUse finalProcesDeFirma(String transactionID, SignaturesSetWeb ss,
        ...
        switch (sss.getStatus()) {
                        ...
                        if (status.getSignedData() == null || !status.getSignedData().exists()) {
                            ...
                            try {
                                ...
                                if (custInfo != null) {
                                    ...
                                    String fitxerAFirmarNom = ssf.signaturesSet.getSignaturesSetID() + "_" + signID;

                                    PeticioDeFirmaJPA peticioDeFirma = peticioFirmaBySignID.get(signID);

                                    IPortaFIBDataSource originalFile = new FileDataSource(
                                            getFitxerAdaptatPath(transactionID, signID));

                                    custodiaInfoLogicaEjb.custodiaThingToDoOnFinishPeticioDeFirma(fitxerAFirmarNom,
                                            originalFile, firmat, peticioDeFirma, firma, custInfo);
                                }
            ...
            case StatusSignaturesSet.STATUS_CANCELLED:
                if (sss.getErrorMsg() == null) {
                    sss.setErrorMsg(I18NLogicUtils.tradueix(new Locale(ss.getCommonInfoSignature().getLanguageUI()),
                            "plugindefirma.cancelat"));
                }
            //statusFinal = sss;
            break;
        ...
        if (ssf.getStatus() == StatusSignaturesSet.STATUS_FINAL_OK) {
            if (signaturesValides < ss.getFileInfoSignatureArray().length) {
                descripcio = "Signatures realitzades correctament: " + signaturesValides + " de "
                        + ss.getFileInfoSignatureArray().length;
            } else {
        ...
        bitacolaLogicaEjb
                .createBitacola(InfoBitacola.builder(ssf.getEntitatID()).objecteid(transactionID)
                        .tipusObjecte(ConstantsV2.BITACOLA_TIPUS_FIRMASINCRONA)
                        ...
                        .descripcio(descripcio).objecte(statusList).build());
    ...
    protected ValidacioCompletaResponse validateSignature(PassarelaSignaturesSetWebInternalUse ssf, File fitxerFirmat,
        ...
        final IPortaFIBDataSource fitxerOriginal;
        {
            try {
                ...
                if (isDebug) {
                    ...
                    log.info("fis.getFileToSign().getFitxerID() => " + fis.getFileToSign().getFitxerID());
                }
        ...
        final IPortaFIBDataSource signature = new es.caib.portafib.logic.utils.datasource.FileDataSource(fitxerFirmat);
        ...
        IPortaFIBDataSource adaptat = new FileDataSource(
                getFitxerAdaptatPath(ssf.getSignaturesSet().getSignaturesSetID(), signID));
    ...
    public File getFitxerOriginalPath(String transactionID, String signID) {
        File p = new File(passarelaBasePath + File.separatorChar + transactionID + File.separatorChar + signID);
        p.mkdirs();
    ...
    protected void deleteSignaturesSet(PassarelaSignaturesSetWebInternalUse pss) {
        ...
        try {
            ...
            log.error("Error eliminant directori " + basePath + "(S'ha d'esborrar manualment): " + e.getMessage(), e);
        }
