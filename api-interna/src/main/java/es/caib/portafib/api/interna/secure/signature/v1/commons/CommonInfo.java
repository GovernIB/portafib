...
public class CommonInfo {
    ...
    @Schema(
            description = " - FIRMA WEB: Requerit. És el codi d'usuari dins l'entitat."
                    ...
                    + " -FIRMA EN SERVIDOR: Opcional. Es reconama que valgui null a no ser que l'administrador digui el contrari."
                    + " És la configuració de firma en el sistema específic de firma. Per exemple amb el Plugin"
                    + " de @firma federat et pots connectar amb un usuari-password però aquest pot tenir diverses"
                    + " configuracions per fer firmes en servidor o àlies: \"username\" s'utilitza de"
                    + " definir aquesta configuració o àlies.",
            example = "\"u800xx\" / \"u[DNI]\"",
            requiredMode = RequiredMode.NOT_REQUIRED)
    protected String username;

    @Schema(
            description = " - FIRMA WEB: Requerit. És el DNI de la persona signant. Si esta activa la validació dins"
                    + " PortaFIB llavors es valida que el DNI del Certificat sigui el mateix que aquest.\r\n"
                    + " - FIRMA EN SERVIDOR: Opcional. És el CIF o NIF associat al certificat en servidor."
                    + " Si es defineix i si esta activa la validació dins PortaFIB llavors es valida que el"
                    + " DNI del Certificat sigui el mateix que aquest.",
            example = "99999999Z",
            requiredMode = RequiredMode.REQUIRED)
    protected String administrationID;

    @Schema(
            description = "Opcional. És el CIF de l'organització representada pel signant. Si esta activa la validació"
                    + " dins PortaFIB llavors es valida que el Certificat sigui un certificat de representant d'aquest CIF.",
            example = "B99999999",
            requiredMode = RequiredMode.NOT_REQUIRED)
    ...
    public CommonInfo(String signProfile, String languageUI, String username, String administrationID,
            String signerEmail) {
        super();
