...
public class RestUtils {
	...
	/*
	 * public static boolean authenticateUsernamePassword(HttpServletRequest
	 * request, String username, String password, Set<String> roles, Logger log) {
	 * 
	 * try { LoginContext lc = new LoginContext("seycon", new
	 * PassiveCallbackHandler(username, password)); lc.login();
	 * 
	 * Set<Principal> principalsCred = lc.getSubject().getPrincipals(); for
	 * (Principal object : principalsCred) { if ("Roles".equals(object.getName()) &&
	 ...
	 * while(enumPrinc.hasMoreElements()) { Principal rol = enumPrinc.nextElement();
	 * roles.add(rol.getName()); } } }
	 * 
	 * return true;
	 * 
	 * } catch (Exception e) { log.info("Login ERROR: " + e.getMessage(), e); return
	 * false; } }
	 */
