    ...
    public ResponseEntity<?> upgradeSignature(HttpServletRequest request, @RequestBody FirmaSimpleUpgradeRequest fsur) {
        ...
        try {
            ...
            if (singTypeForm == null) {
                // XYZ ZZZ Traduir
                return generateServerError("El identificador d'Extensió de Firma " + upgradeID + " no existeix.");
            }
            ...
            upgradeResponse = passarelaDeFirmaEnServidorEjb.upgradeSignature(signature, fsur.getDetachedDocument(),
                    fsur.getTargetCertificate(), singTypeForm, loginInfo.getUsuariAplicacio(), perfilDeFirma, config,
    ...
    public ResponseEntity<?> signDocument(HttpServletRequest request,
        ...
        try {
            ...
            pcf = configuracioUsuariAplicacioLogicaLocalEjb.getConfiguracioFirmaPerApiFirmaSimpleEnServidor(
                    loginInfo.getUsuariAplicacio().getUsuariAplicacioID(), perfil.getCodi(), simpleSignature);
